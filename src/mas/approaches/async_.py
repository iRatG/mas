# -*- coding: utf-8 -*-
"""Асинхронный подход (перенос из approach2_async.py)."""
...
from mas.llm.mock_client import llm_find_issues, llm_suggest_fixes, llm_review_fix
...
class Coordinator(AsyncAgent):
	...
	async def run_case(self, code: str, bug_id: int, cid: str) -> Dict[str, Any]:
		...
		if report is None:
			...
			return {"status": "timeout", "approach": "async", "cid": cid, "metrics": self.metrics.to_dict()}
		...
		self.metrics.end_time = time.time()
		return {"status": "failed", "approach": "async", "cid": cid, "bug_report": report, "metrics": self.metrics.to_dict()}
