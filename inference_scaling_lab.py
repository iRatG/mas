...
def run_experiment(approach: str, seed: int, n_candidates: int, n_reviewers: int, max_retries: int, cases: List[int] = None) -> Dict[str, Any]:
    """
    Запуск одного эксперимента с заданными параметрами.
    
    Args:
        approach: 'sync' или 'async'
        seed: Seed для воспроизводимости
        n_candidates: Количество кандидатов
        n_reviewers: Количество ревьюеров
        max_retries: Максимальное количество ретраев
        cases: Список кейсов для тестирования (по умолчанию все)
...
def parse_output(stdout: str) -> Dict[str, Any]:
    """
    Парсинг вывода main.py для извлечения ключевых метрик.
    """
...
def run_scaling_experiment() -> List[Dict[str, Any]]:
    """
    Запуск полного эксперимента по инференс-скейлингу.
    """
...
def save_results(results: List[Dict[str, Any]], analysis: Dict[str, Any]):
    """
    Сохранение результатов в CSV и JSON файлы.
    """
